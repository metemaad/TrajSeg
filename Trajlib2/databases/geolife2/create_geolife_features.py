...
import pandas as pd
import numpy as np
import Trajlib2.databases.geolife2.TrajectorySegmentation as ts
import Trajlib2.databases.geolife2.Trajectory as tr
...
ts_obj.load_data(lat='latitude', lon='longitude', time_date='collected_time',
                 labels=['transportation_mode'], src='geolife_full.csv', seperator=',')
...
for seg in range(len(trajectorySegments)):
    ...
    if trajectorySegments[seg].shape[0] > 10:
        ...
        user_id = trajectorySegments[seg].t_user_id.values[0]
        print(user_id)
