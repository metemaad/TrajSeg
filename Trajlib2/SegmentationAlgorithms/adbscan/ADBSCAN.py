from Trajlib2.SegmentationAlgorithms.adbscan.cluster import Cluster
from Trajlib2.SegmentationAlgorithms.adbscan.segment import Segment
...
class ADBSCAN:
    ...
    def cluster_trajectory_point(self, tp):
        ...
        if self.Status == "clustering":
            if self.current_trajectory_point is not None:
                print("continue online clustering... curr p is [", str(self.current_trajectory_point.latitude), ",",
                      str(self.current_trajectory_point.longitude) + "]")
            else:
                print("process first point")
            if self.window_buffer.cluster(self.current_trajectory_point):
                # tp belongs to cluster
                ...
                # wait for next point to process

            else:
                # tp does not belong to curr cluster

                # count number of points is curr buffer
                c = self.current_cluster.get_cardinality()
                print("cardinality of curr cluster:",str(c))
                if c <= self.min_points:
                    ...
                    if self.current_cluster.trajectory_points is not None:
                        for tp0 in self.current_cluster.trajectory_points:
                            # remove noises from the buffer **************************
                            self.noise_cluster.assign_point(tp0)
                        # if < minpoint add them to noise cluster
                    ...
                    # if >minpoint consider them as valid cluster
