...
class SWSExperiment:
    ...
    def tuning_step(tuning, **kwargs):
        ...
        seg_id, best_parameters = s.segment_by_sws(tuning_set=tuning, tuning_set_ground_truth=(tsid, label), **kwargs)
        return best_parameters, s

    @staticmethod
    def testing(test_trajectory, s, best_parameters):
        s = TrajectorySegmentation(test_trajectory)
        seg_id, best_parameters = s.segment_by_sws(**best_parameters, verbose=False)
    ...
    def reporting(self, testing_set, best_parameters, i):
        ...
        r = segmentation_evaluation.report(segments=segments, label=label, tsid=TSid, verbose=False)
    ...
    def save_results(self, results_of_evaluation, track_memory_array):
        ...
        df = pd.DataFrame(results_of_evaluation, columns=['Purity', 'Coverage', 'Harmonic mean', 'best parameters',
                                                          'k_segment1', 'k_segment2', 'k_tsid', 'acc', 'kappa', 'pr', 're', 'j', 'p2', 'c2', 'h2','homo','comp','v_measure'])
