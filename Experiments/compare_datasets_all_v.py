...
#
    ...
    axs = sns.boxplot(ax=axs,x="DataSet", y='v_measure',
                         hue="Algorithms", data=df, orient="v", notch=False)
    axs.set_ylim([0,100])
    axs.set_ylabel('v_measure')
    ...
    fig.suptitle("Compare SWS with other algorithms on three Datasets")
    #axs[0].ylim([50,100])
    ...
    plt.savefig('SWS_compare_other_v_measure.png')
...
for _,__ in dfh.groupby(['DataSet']):
    ...
    for k, v in __.groupby('Algorithms'):
        if k!=abest[0]:
            ...
            s,p=stats.ttest_ind(abest[1],v['v_measure'].values)
            if (p<=0.05):
...
for _, __ in dfh.groupby(['DataSet']):
    ...
    for k1, v1 in __.groupby('Algorithms'):
        print(k1,'mdn:%0.2f' % v1['v_measure'].median(),
                  'iqr:%0.2f' % iqr(v1['v_measure']))
        for k, v in __.groupby('Algorithms'):
            p, s = kruskal(v1['v_measure'], v['v_measure'])
            print(_, k1, k, 'mdn:%0.2f' % v['v_measure'].median(),
                  'iqr:%0.2f' % iqr(v['v_measure']), 's:%0.5f' % p, 'p:%0.5f' % s)
