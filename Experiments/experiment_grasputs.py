...
class GRASPUTSExperiment:
    ...
    def tuning_step(tuning, **kwargs):
        ...
        seg_id, best_parameters = s.segment_by_grasputs(tuning_set=tuning, tuning_set_ground_truth=(tsid, label), **kwargs)
        return best_parameters, s

    @staticmethod
    def testing(test_trajectory, s, best_parameters):
        s = TrajectorySegmentation(test_trajectory)
        seg_id, best_parameters = s.segment_by_grasputs(**best_parameters, verbose=False)
