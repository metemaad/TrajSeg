...
#
    ...
    axs[0] = sns.boxplot(ax=axs[0],x="DataSet", y='Purity',
                         hue="Algorithms", data=df, orient="v", notch=False)
    axs[0].set_ylim([75,100])
    axs[0].set_ylabel('')

    axs[1] = sns.boxplot(ax=axs[1],x="DataSet", y='Purity',
                hue="Algorithms", data=df,orient="v",notch=False)
    ...
    axs[0].axvspan(xmin=-0.40, xmax=-0.08, color='g', alpha=0.2)#swsfishing
    ...
    plt.savefig('SWS_compare_other.png')

    plt.show()


dfh,xcl=load_df('Purity')
...
for _,__ in dfh.groupby(['DataSet']):
    ...
    for k, v in __.groupby('Algorithms'):
        if k!=abest[0]:
            ...
            s,p=stats.ttest_ind(abest[1],v['Purity'].values)
            if (p<=0.05):
