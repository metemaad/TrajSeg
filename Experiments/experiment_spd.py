...
import psutil
import random
from Experiments.plot import plot_segments
...
class SPDExperiment:
    ...
    def tuning_step(tuning, **kwargs):
        ...
        seg_id, best_parameters = s.segment_by_spd(tuning_set=tuning, tuning_set_ground_truth=(tsid, label), **kwargs)
        return best_parameters, s

    @staticmethod
    def testing(test_trajectory, s, best_parameters):
        s = TrajectorySegmentation(test_trajectory)
        seg_id, best_parameters = s.segment_by_spd(**best_parameters, verbose=False)
    ...
    def __main__(self, **kwargs):
        ...
        for i in listOfDatasets:
            ...
            print("kp:",result_data[5],'k:',result_data[4],result_data)
