...
#
    ...
    axs = sns.boxplot(ax=axs,x="DataSet", y='Coverage',
    ...
    plt.savefig('SWS_compare_other_c.png')

    plt.show()


dfh,xcl=load_df('Coverage')

plot(dfh,'Coverage',xcl)
for _,__ in dfh.groupby(['DataSet']):
    ...
    for k, v in __.groupby('Algorithms'):
        if k!=abest[0]:
            ...
            s,p=stats.ttest_ind(abest[1],v['Coverage'].values)
            if (p<=0.05):
