...
class WKmeansExperiment:
    ...
    def tuning_step(tuning, **kwargs):
        ...
        seg_id, best_parameters = s.segment_by_wkmeans(tuning_set=tuning, tuning_set_ground_truth=(tsid, label), **kwargs)
        return best_parameters, s

    @staticmethod
    def testing(test_trajectory, s, best_parameters):
        s = TrajectorySegmentation(test_trajectory)
        seg_id, best_parameters = s.segment_by_wkmeans(**best_parameters, verbose=False)
