...
def load_df(col='Harmonic mean'):
    ...
    df2 = pd.read_csv('HurricanesDataset/Results_HurricanesDataset_WSII_0.9DT.csv')
    df2 = pd.DataFrame(df2[col].copy())
    df2 = df2.assign(DataSet=['Hurricanes'] * df2.shape[0])
    df2 = df2.assign(Algorithms=['WSII'] * df2.shape[0])
    df = df.append(df2)

    df2 = pd.read_csv('GeolifeDataset/Results_GeolifeDataset_WSII_0.9DT.csv')
    df2 = pd.DataFrame(df2[col].copy())
    df2 = df2.assign(DataSet=['Geolife'] * df2.shape[0])
    df2 = df2.assign(Algorithms=['WSII'] * df2.shape[0])
    df = df.append(df2)
...
#
    ...
    plt.savefig('WSII_compare_other_v_measure.png')
