...
def load_df(col='Harmonic mean'):
    ...
    df2 = pd.read_csv('Results_FishingDataset_SWS_C_WS_13.csv')
    df2=pd.DataFrame(df2[col].copy())
...
def plot(df,label,xcl):
    ...
    plt.savefig('SWS_Fishing_ws.png')
...
for _,__ in dfh.groupby(['Algorithms']):
    ...
    #print(_,__['Harmonic mean'].values,__['ws'].values)
